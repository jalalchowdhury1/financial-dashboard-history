...
>
> **N/A resilience (added 2026-06-05):** three layers minimise blank/`N/A` cells going forward — (1) **multi-fetch merge** (`fetch_merged`): each endpoint is fetched a couple of times and merged keeping the first non-null per field, recovering transient per-source nulls; (2) **history fallback** (`cur_or_hist`): if a metric's `current` is null, use the last point of its `history` array (handles both `price` and `value` keys); (3) **carry-forward** (`build_last_known`/`apply_fallbacks`): anything still `N/A` reuses the last known-good value from the sheet (logged each run). Retired LEI (col I) is written **blank**, never carried, never `N/A`. Carried values are re-persisted, so a *sustained* upstream outage (e.g. `copperGold`, whose copper/gold source is currently down) will look frozen-but-current — watch the run logs for repeated `carried forward` lines; the real fix for those is upstream in the dashboard repo.
